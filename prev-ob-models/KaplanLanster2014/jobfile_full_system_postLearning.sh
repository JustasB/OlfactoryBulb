...
echo "Starting pattern $PN at `date`"
aprun -n 120 /cfs/klemming/nobackup/b/bkaplan/OlfactorySystem/neuron_files/x86_64/special -mpi  -c "x=$PN" -c "strdef param_file" -c "sprint(param_file, \"$PARAM_FN\")" /cfs/klemming/nobackup/b/bkaplan/OlfactorySystem/neuron_files/start_file_full_system_recognition_task.hoc > delme_fullSystem_$PN \
echo "Stopping pattern $PN at `date`"
done
