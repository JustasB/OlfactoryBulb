...
## change the morphproc command far below to have pulseinresp1, pulseinresp2, etc unique str.
...
if FORCE_SALIENT:
    ...
    stim_seed_list = arange(750.0,800.0,1.0)#[157.0,160.0,190.0,191.0,212.0,441.0]
    ...
    for stimseed in stim_seed_list:
        ...
        for netseed in net_seed_list:
            ...
            for inh in inh_options:
                ...
                stim_frate_filename = '../generators/firerates/firerates_pulseinresp_'+str(stimseed)
                stim_frate_filename += '.pickle'
                if not os.path.exists(stim_frate_filename):
                    print "Generating firerate file",stim_frate_filename
                    gen_command = 'python2.6 '+cwd+'/../generators/generate_firerates_odors.py PULSEINRESP NOSHOW'
                    subprocess.check_call(gen_command,shell=True)
                else:
                ...
                if not os.path.exists(stim_firefiles_dirname+\
                        '/firetimes_pulseinresp_glom_'+str(num_gloms-1)+\
                        '_odorB_avgnum0.txt'):
                    print "Generating firefiles in",stim_firefiles_dirname
                    gen_command = 'python2.6 '+cwd+'/../generators/generate_firefiles_odors.py PULSEINRESP NOSHOW'
                    subprocess.check_call(gen_command,shell=True)                
                else:
                ...
                ## odor_pulseinresp.py checks if the output files exists
                ## if there is already an output file, it quits.
                morphproc = subprocess.Popen('mpiexec -machinefile ~/hostfile -n 31'\
                    ' ~/Python-2.6.4/bin/python2.6 odor_pulseinresp.py NOSHOW pulseinresp1',\
