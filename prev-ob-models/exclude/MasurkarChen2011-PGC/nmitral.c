...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 4);
 	...
 	ivoc_help("help ?1 na e:/Dropbox/Research/Thesis/MODELS/MasurkarChen2011/nmitral.mod\n");
...
/*CVODE*/
 ...
 static void _check_trates() {
  ...
  static double _sav_Am1;
  static double _sav_Am2;
  static double _sav_Rd;
  static double _sav_Rg;
  static double _sav_thm1;
  static double _sav_thm2;
  static double _sav_thi1;
  static double _sav_thi2;
  static double _sav_qm1;
  static double _sav_qm2;
  static double _sav_qi;
  static double _sav_qinf;
  static double _sav_thinf;
  if (!usetable) {return;}
  if (_sav_dt != dt) { _maktable = 1;}
  if (_sav_celsius != celsius) { _maktable = 1;}
  if (_sav_temp != temp) { _maktable = 1;}
  if (_sav_Am1 != Am1) { _maktable = 1;}
  if (_sav_Am2 != Am2) { _maktable = 1;}
  if (_sav_Rd != Rd) { _maktable = 1;}
  if (_sav_Rg != Rg) { _maktable = 1;}
  if (_sav_thm1 != thm1) { _maktable = 1;}
  if (_sav_thm2 != thm2) { _maktable = 1;}
  if (_sav_thi1 != thi1) { _maktable = 1;}
  if (_sav_thi2 != thi2) { _maktable = 1;}
  if (_sav_qm1 != qm1) { _maktable = 1;}
  if (_sav_qm2 != qm2) { _maktable = 1;}
  if (_sav_qi != qi) { _maktable = 1;}
  if (_sav_qinf != qinf) { _maktable = 1;}
  if (_sav_thinf != thinf) { _maktable = 1;}
...
static int  rates (  double _lvm ) {
   double _la , _lb ;
 _la = trap0 ( _threadargscomma_ _lvm , thm1 , Am1 , qm1 ) ;
   _lb = trap0 ( _threadargscomma_ - _lvm , - thm2 , Am2 , qm2 ) ;
   mtau = 1.0 / ( _la + _lb ) ;
   minf = _la * mtau ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 91 in file nmitral.mod:\n        SOLVE states  METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
