...
<xsl:variable name="drawGreyCells" select="false()"/>
<!-- For anaglyph, draw only white on black. -->
<xsl:variable name="forAnaglyph" select="true()"/>
...
<xsl:template name="setColor">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:variable name="color">1 1 1</xsl:variable>
        </xsl:otherwise>
    </xsl:choose>
...
<xsl:template name="setColorByName">
    ...
    <xsl:choose>
                ...
                <xsl:choose> <!-- R, G, B for gloms 0 to 3 (and cycled), in two shades for two sisters each. Transparent mild gray for others -->
                    <xsl:when test="$popName='mitrals'">
                        <xsl:choose>
                            ...
                            <xsl:when test="$cellId=2">0<xsl:text> </xsl:text>1<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text></xsl:when>
                            <xsl:when test="$cellId=3">1<xsl:text> </xsl:text>1<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text></xsl:when>
                            <xsl:when test="$cellId=4">0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>1<xsl:text> 1</xsl:text></xsl:when>
                            <xsl:when test="$cellId=5">0<xsl:text> </xsl:text>1<xsl:text> </xsl:text>1<xsl:text> 1</xsl:text></xsl:when>
        ...
        <xsl:otherwise>
            <xsl:variable name="colorRGBA">1 1 1</xsl:variable>
        </xsl:otherwise>
    </xsl:choose>
