function xdot = vfield_ET_MCRI_pexcite(t,x,p)
...
xdot = zeros(size(x));
xdot(1) = -(ET_INa + ET_IK + ET_ICaT + ET_IH + ET_INaP + ET_IL + p.ET_Iext - p.ET_gORN*Input)./p.ET_C;
...
xdot(N+1) = -(MC_INa + MC_INaP + MC_IKa + MC_IKfast + MC_IKslow + MC_IL + p.MC_Iext + MC_Isyn + MC_GC_recurrent - p.MC_gORN*Input);
