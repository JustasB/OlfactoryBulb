...
if __name__ == '__main__':
    ...
    noisefree_folder = os.path.abspath(sys.argv[1]) + '/' # this should include all the data ready to run a full system
    print 'Taking data from:', noisefree_folder 

    srcs = []
    srcs.append('%sParameters/' % noisefree_folder)
    srcs.append('%sConnections/' % noisefree_folder)
    for src in srcs:
    ...
    noisefree_activation_matrix_fn = noisefree_folder + 'Parameters/activation_matrix.dat'
    OrnParamClass = CreateOrnParameters.CreateOrnParameters(params) # patterns for ORN activation must be recreated to add noise
    ok = OrnParamClass.create_single_odorant_patterns(given_activation_matrix=noisefree_activation_matrix_fn)
