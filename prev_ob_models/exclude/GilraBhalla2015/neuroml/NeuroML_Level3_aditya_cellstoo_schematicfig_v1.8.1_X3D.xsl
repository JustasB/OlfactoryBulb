...
<X3D profile="Immersive" version="3.1" xmlns:xsd='http://www.w3.org/2001/XMLSchema-instance' xsd:noNamespaceSchemaLocation=' http://www.web3d.org/specifications/x3d-3.1.xsd '>
    ...
    <Scene>
        ...
        <!-- obsolete <Viewpoint description="for joints connections"	position="715.9, -558.3, 1306" orientation="-0.732 -0.2904 -0.6163 5.081"/> -->
        <!-- obsolete <Viewpoint description="for joints connections"	position="-245.5, -850.5, 1733" orientation="-0.9313, 0.1289, 0.3407, 5.58"/>-->
        <Viewpoint description="for joints connections"	position="-256.5, -973.8, 1258" orientation="-0.9766, 0.08087, 0.199, 5.259"/>
        ...
        <xsl:choose> <!-- R, G, B for gloms 0 to 3 (and cycled), in two shades for two sisters each. Transparent mild gray for others -->
            <xsl:when test="$popName='mitrals'">
                <xsl:choose>
                    ...
                    <xsl:when test="$cellId=1">0.5<xsl:text> </xsl:text>0<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text></xsl:when>
                    <xsl:when test="$cellId=4">0<xsl:text> </xsl:text>1<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text></xsl:when>
                    <xsl:when test="$cellId=5">0<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text></xsl:when>
                    <xsl:when test="$cellId=2">0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>1<xsl:text> 1</xsl:text></xsl:when>
                    <xsl:when test="$cellId=3">0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>0.5<xsl:text> 1</xsl:text></xsl:when>
                    <xsl:otherwise>0.5<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0.2</xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <!-- obsolete: specific joint from mit 2 to mit 0 for 751.0 netseed and 0.05 frac_directed -->
            <!--<xsl:when test="($popName='granules_joints') and ($cellId=1160)">
                0<xsl:text> </xsl:text>1<xsl:text> </xsl:text>0<xsl:text> 1</xsl:text>
            </xsl:when>-->
            <!-- obsolete: specific joint from mit 4 to mit 1 for 751.0 netseed and 0.05 frac_directed -->
            <!--<xsl:when test="($popName='granules_joints') and ($cellId=444)">
                0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>1<xsl:text> 1</xsl:text>
            </xsl:when>-->
            <!-- colours from http://cloford.com/resources/colours/500col.htm -->
            <xsl:when test="($popName='granules_joints')"> <!-- all other joints -->
                <xsl:text>1 0.5 0 0.6</xsl:text> <!-- orange -->
            </xsl:when>
            <xsl:when test="($popName='granules_multis')"> <!-- all multis -->
                <xsl:text>0.55 0.55 0.22 0.6</xsl:text> <!-- olive drab -->
            </xsl:when>
            <xsl:when test="($popName='granules_singles')"> <!-- all singles -->
                <xsl:text>0.55 0.22 0.55 0.6</xsl:text> <!-- beet -->
            </xsl:when>
...
<xsl:template name="setColorByConn">
    ...
    <xsl:variable name="colorRGBA">
        <xsl:choose> <!-- color as per other mitral color -->
            <xsl:when test="$preCellId=0">1<xsl:text> </xsl:text>0<xsl:text> </xsl:text>0<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:when test="$preCellId=1">0.5<xsl:text> </xsl:text>0<xsl:text> </xsl:text>0<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:when test="$preCellId=4">0<xsl:text> </xsl:text>1<xsl:text> </xsl:text>0<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:when test="$preCellId=5">0<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:when test="$preCellId=2">0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>1<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:when test="$preCellId=3">0<xsl:text> </xsl:text>0<xsl:text> </xsl:text>0.5<xsl:text> 0.4</xsl:text></xsl:when>
            <xsl:otherwise>0.5<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0.5<xsl:text> </xsl:text>0.2</xsl:otherwise>
        </xsl:choose>
        <!--<xsl:choose>-->
            <!-- special colors to highlight a granule cell is obsolete now. -->
